...
# KONFIGURASI
# ════════════════════════════════
st.set_page_config(page_title="Sistem Prediksi Kurs", layout="wide")
...
# ── Load model Prophet (.pkl) per mata uang
@st.cache_resource
...
# ── Ambil kurs realtime dari ExchangeRate-API
...
# ── Load data aktual Bank Indonesia (untuk validasi prediksi 1-9 Jan 2026)
@st.cache_data
def load_actual_data(currency):
    ...
    try:
        ...
        df["y_actual"] = df["Kurs Jual"] / df["Nilai"]  # normalisasi (mis. JPY per 100 -> per 1)
...
# ── Hasilkan data historis + forecast 7 hari ke depan
...
# ── Komponen: Insight Prediksi
def generate_forecast_insight(selected, forecast_df, realtime):
    first_val  = forecast_df["yhat"].iloc[0]
    last_val   = forecast_df["yhat"].iloc[-1]
    ...
    if abs(pct_change) <= 0.3:
        insights.append(
            f"🔹 Prediksi menunjukkan **{selected}/IDR cenderung stabil** dalam 7 hari ke depan "
            f"dengan perubahan hanya **{pct_change:+.2f}%**."
        )
    elif pct_change > 0:
        insights.append(
            f"🔹 Prediksi menunjukkan **{selected}/IDR cenderung menguat {pct_change:.2f}%** "
            f"dalam 7 hari ke depan (IDR melemah terhadap {selected})."
        )
    else:
        insights.append(
            f"🔹 Prediksi menunjukkan **{selected}/IDR cenderung melemah {abs(pct_change):.2f}%** "
            f"dalam 7 hari ke depan (IDR menguat terhadap {selected})."
        )

    insights.append(
        f"🔹 Tren prediksi bergerak dalam rentang **Rp{min_pred:,.0f} – Rp{max_pred:,.0f}**, "
        f"dengan rata-rata prediksi **Rp{avg_pred:,.2f}**."
    )

    if realtime:
        diff = avg_pred - realtime
        diff_pct = (diff / realtime) * 100
        if abs(diff_pct) < 1:
            insights.append(
                f"🔹 Rata-rata prediksi **mendekati kurs realtime** saat ini (Rp{realtime:,.2f}), "
                f"selisih hanya **{diff_pct:+.2f}%**."
            )
        elif diff > 0:
            insights.append(
                f"🔹 Rata-rata prediksi **di atas kurs realtime** (Rp{realtime:,.2f}) sebesar "
                f"**Rp{abs(diff):,.2f} ({abs(diff_pct):.2f}%)** — kurs diprediksi akan naik."
            )
        else:
            insights.append(
                f"🔹 Rata-rata prediksi **di bawah kurs realtime** (Rp{realtime:,.2f}) sebesar "
                f"**Rp{abs(diff):,.2f} ({abs(diff_pct):.2f}%)** — kurs diprediksi akan turun."
            )
...
st.markdown(
    """
    <div style='text-align: justify; color: rgba(250,250,250,0.6); font-size: 0.875rem; line-height: 1.5; margin-bottom: 1rem;'>
...
with col2:
    st.markdown(
        """
        <div style='font-size:14px; line-height:1.6; margin-bottom:0.4rem; color:rgb(250,250,250);'>
            konversi
        </div>
        """,
        unsafe_allow_html=True
...
with col3:
    st.markdown(
        f"""
        <div style='font-size:22px;font-weight:bold;margin-top:2.4rem'>
            {from_curr} → {to_curr}
        </div>
        """,
        unsafe_allow_html=True
...
if model is None:
    ...
    st.markdown(
        ...
        '>
            ...
            <span style='color: {warna}; font-weight: 700; font-size: 1rem'>{label_tren}</span>
            <span style='color: {warna}; font-size: 0.9rem'>({pct_str} dalam 7 hari)</span>
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    # ── Grafik 1: Prediksi 7 Hari Ke Depan (Plotly)
    ...
    insights = generate_forecast_insight(selected, forecast_df, realtime)
    for insight in insights:
        st.markdown(insight)
    
    st.divider()

    # ── Grafik 2: Validasi Aktual (Bank Indonesia) vs Prediksi
    ...
    if actual_df is not None and not actual_df.empty:
        ...
        if not validasi_df.empty:
            ...
            # MAPE khusus periode validasi ini
            mape_periode = (
                (validasi_df["y_actual"] - validasi_df["yhat"]).abs() / validasi_df["y_actual"]
            ).mean() * 100
            st.caption(
                f"Perbandingan dilakukan menggunakan data aktual Bank Indonesia dengan rentang waktu 7 hari kerja."
            )

            missing = set(forecast_df["ds"]) - set(validasi_df["ds"])
            if missing:
                missing_str = ", ".join(d.strftime("%d %b") for d in sorted(missing))
                st.caption(f"ℹ️ Tanggal {missing_str} tidak tersedia di data BI (hari libur), tidak diikutkan dalam validasi.")
        else:
            st.info("Tidak ada tanggal yang cocok antara data prediksi dan data aktual BI.")
    else:
        st.info(f"Data aktual BI untuk {selected} periode ini belum tersedia.")

    st.divider()

    # ── Grafik 3: Historis (Plotly)
    ...
    with st.expander("Informasi Model", expanded=False):
        ...
        with col_m1:
            st.markdown("<div style='text-align:center'><b>Algoritma</b></div>", unsafe_allow_html=True)
            st.markdown("<div style='text-align:center'>Prophet (Meta)</div>", unsafe_allow_html=True)
        with col_m2:
            st.markdown("<div style='text-align:center'><b>Periode</b></div>", unsafe_allow_html=True)
            st.markdown(f"<div style='text-align:center'>{tgl_awal} – {tgl_akhir}</div>", unsafe_allow_html=True)
        with col_m3:
            st.markdown("<div style='text-align:center'><b>Observasi</b></div>", unsafe_allow_html=True)
            st.markdown(f"<div style='text-align:center'>{jumlah_data:,} data</div>", unsafe_allow_html=True)
 
        st.markdown("---")
        st.markdown(
                   """
            <div style='text-align: justify; line-height: 1.5; margin-bottom: 1rem;'>
